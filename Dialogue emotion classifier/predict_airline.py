...
from datasets import create_airline_dataset
...
def main():
    os_dataset, N = create_airline_dataset(tokenizer, batch_size, max_length)
    ...
    np.save('../airline_data/preds.npy', preds)

def get_confidence_values():
    preds = np.load('../airline_data/preds.npy')
    dialog_ids = np.load('../airline_data/dialog_ids.npy')
    ...
    df.to_csv('../airline_data/airline_prediction.csv')
