...
from datasets import create_media_dataset
...
def main():
    os_dataset, N = create_media_dataset(tokenizer, batch_size, max_length)
    ...
    emobert.load_weights('roberta2emobert.h5')
    ...
    np.save('../media_data/preds.npy', preds)

def get_confidence_values():
    preds = np.load('../media_data/preds.npy')
    dialog_ids = np.load('../media_data/dialog_ids.npy')
    ...
    df.to_csv('../media_data/media_prediction.csv')
