...
from datasets import create_linux_dataset
...
def main():
    os_dataset, N = create_linux_dataset(tokenizer, batch_size, max_length)
    ...
    np.save('../linux_data/preds.npy', preds)

def get_confidence_values():
    preds = np.load('../linux_data/preds.npy')
    dialog_ids = np.load('../linux_data/dialog_ids.npy')
    ...
    df.to_csv('../linux_data/linux_prediction.csv')
